...
# -*- coding: utf-8 -*-
"""
上传图片和资源文件到 GitHub - 可指定Token的版本
用法: python upload_files_with_token.py <github_token>
"""
...
def get_file_sha(filename, github_token):
    """获取 GitHub 上该文件的 SHA"""
...
def upload_file(filename, file_path, github_token, retry_count=0, max_retries=3):
    """上传单个文件到 GitHub"""
    api_base = f"https://api.github.com/repos/{GITHUB_OWNER}/{GITHUB_REPO}/contents"
    url = f"{api_base}/{TARGET_DIR}/{filename}"
    ...
    try:
        ...
        sha = get_file_sha(filename, github_token)
        ...
        if response.status_code in [200, 201]:
            ...
            time.sleep(1)
            return upload_file(filename, file_path, github_token, retry_count + 1, max_retries)
        else:
...
def main(github_token):
    ...
    masked_token = github_token[:7] + "*" * (len(github_token) - 14) + github_token[-7:] if len(github_token) > 14 else "***"
    ...
    for idx, (filename, file_path) in enumerate(files, 1):
        ...
        if upload_file(filename, file_path, github_token):
            success_count += 1
        else:
            failed_count += 1
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("❌ 用法: python upload_files_with_token.py <github_token>")
        print("例如: python upload_files_with_token.py ghp_xxxxxxxxxxxxxxxxxxxx")
        sys.exit(1)
    
    github_token = sys.argv[1].strip()
    if not github_token:
        print("❌ 错误：GitHub Token 不能为空")
        sys.exit(1)
